...
public class my_algorithm_without_mutation {
		...
		while(TOTAL_EVALUATION < MAX_EVALUATION) {
			...
			for(int i=0;i<N;i++){
				...
				if(p1<ratio_mut_crs){
					...
					if(imp == true){
						System.out.println("Improvement: "+getKey(res1)+" | "+getKey(newRes));
						mut_sucess++;
					}
					else{
			...
			//int imp_in_currentIteration = (improvements+map.size()) - imp_in_prevIteration;
...
//
		File file = new File("myalgo_womutation.csv");
